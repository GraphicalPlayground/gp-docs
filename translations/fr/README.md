...
Si vous découvrez une vulnérabilité de sécurité dans `gp-docs`, veuillez ne pas la signaler en ouvrant un issue public. Référez-vous plutôt à notre [Politique de Sécurité](./SECURITY.md) pour obtenir des instructions sur la manière de divulguer la vulnérabilité de manière sécurisée aux mainteneurs.

### Licence

`gp-docs` est un logiciel open-source. Veuillez consulter le fichier [LICENSE.md](../../LICENSE.md) dans le répertoire racine pour les termes complets concernant la modification, la distribution et l'utilisation dans vos propres projets.

### Dons

Si vous trouvez `gp-docs` utile pour votre apprentissage, votre recherche académique ou votre parcours dans le développement de jeux, pensez à soutenir le projet. Maintenir un moteur graphique C++ moderne demande un temps et des ressources considérables !
...
Si vous avez des questions, des suggestions ou souhaitez partager vos projets réalisés avec `gp-docs`, nous serions ravis de vous entendre ! Vous pouvez nous contacter via les canaux suivants :
